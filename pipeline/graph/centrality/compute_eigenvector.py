...
OUTPUT_EIGENVECTOR = str(RESULTS_DIR / "host_graph_scores_eigenvector_maxiter1000.json")
...
def main():
    ...
    # =========================================================
    # ALGORITHM 1: Standard Eigenvector Centrality (Approximation)
    # =========================================================
    print("\n--- Running Standard Eigenvector Centrality ---")
    print("Parameters: max_iter=1000, tol=1.0e-6")
    
    start = time.time()
    
    # Verbose parameter controls output/progress info from the parallel jobs
    # Works on both Directed and Undirected
    result_ddf = dask_cugraph.eigenvector_centrality(
        G_mg, 
        max_iter=1000, # can adjust to speed up
        tol=1.0e-6
    )
    print(f"Eigenvector centrality calculated in {time.time() - start:.2f} seconds.")
    ...
    eigenvector_scores = final_df.set_index('vertex')['eigenvector_centrality'].to_dict()
    
    print(f"Saving Eigenvector centrality scores to {OUTPUT_EIGENVECTOR}...")
    with open(OUTPUT_EIGENVECTOR, 'w') as f:
        json.dump(eigenvector_scores, f)
