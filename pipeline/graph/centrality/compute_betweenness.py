...
OUTPUT_BETWEENNESS = str(RESULTS_DIR / "host_graph_scores_betweenness_k1400000.json")
OUTPUT_CURRENT_FLOW = str(RESULTS_DIR / "host_graph_scores_current_flow_k14000.json")
...
def main():
    ...
    # =========================================================
    # ALGORITHM 1: Standard Betweenness Centrality (Approximation)
    # =========================================================
    print("\n--- Running Standard Betweenness Centrality ---")
    print("Parameters: k=1400000, endpoints=False, normalized=True")
    
    start = time.time()
    
    # Verbose parameter controls output/progress info from the parallel jobs
    # Works on both Directed and Undirected
    result_ddf = dask_cugraph.betweenness_centrality(
        G_mg, 
        k=1400000, # was 2000
        normalized=True, 
        random_state=42 # note: cuGraph uses random_state instead of seed
        # backend="parallel" # cuda is faster than parallel cpus
    )
    print(f"Betweenness calculated in {time.time() - start:.2f} seconds.")
    ...
    betweenness_scores = final_df.set_index('vertex')['betweenness_centrality'].to_dict()
    
    print(f"Saving Betweenness scores to {OUTPUT_BETWEENNESS}...")
    with open(OUTPUT_BETWEENNESS, 'w') as f:
        json.dump(betweenness_scores, f)
