...
fn load_tokenizer(tokenizer_name: &String) -> Result<(Tokenizer, usize)> {
    // 1. Try to load directly from a file if it exists
    let tokenizer_path = std::path::Path::new(tokenizer_name).join("tokenizer.json");
    
    let tokenizer = if tokenizer_path.exists() {
        // This is the "Gold Standard" for local loading - no internet involved
        Tokenizer::from_file(&tokenizer_path)
            .map_err(|e| anyhow!("Failed to load local tokenizer.json: {}", e))?
    } else {
        // Fallback to hub if local doesn't exist
        Tokenizer::from_pretrained(tokenizer_name, None)
            .map_err(|e| anyhow!("Failed to load tokenizer from Hub/Path {}: {}", tokenizer_name, e))?
    };

    // 2. Identify vocab size
    let vocab_size = if tokenizer_name.contains("gpt-neox-20b") {
        GPT_NEOX_VOCAB_SIZE
    } else if tokenizer_name.contains("Meta-Llama-3-8B") {
        LLAMA3_VOCAB_SIZE
    } else {
