...
CONFIG_PATH = os.path.join(open_lm_install_dir, "model_configs", "open_lm_411m_v2.json")
...
print("Downloading GPT-NeoX tokenizer...")
...
for folder_name in model_folders:
    ...
    # --- C. Run the Conversion with DYNAMIC RESIZING ---
    try:
        # 1. Load raw weights FIRST
        print("Loading PyTorch weights into memory (CPU)...")
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        raw_state_dict = checkpoint.get("state_dict", checkpoint)

        # 2. Dynamically measure the architecture from the weights and folder name
        config_dict = base_config_dict.copy()

        # Measure Vocab Size and Hidden Dimension (handle module./orig_mod. prefixes defensively)
        embed_key = next(
        ...
        # Extract number of attention heads from the DCLM folder name (e.g., _h=8)
        ...
        if os.path.exists(params_txt_path):
            ...
            if qk_keys:
                ...
                print(f"⚠️  No params.txt found; inferred apply_qk_norm=True from checkpoint ({len(qk_keys)} q/k_norm keys)")
            ...
            if not has_norm_bias:
                ...
                print(f"⚠️  No params.txt found; inferred norm_type=gain_only_lp_layer_norm (no bias in checkpoint norms)")

        print(f"📏 DYNAMIC RESIZE: dim={config_dict.get('dim')}, layers={config_dict.get('n_layers')}, heads={config_dict.get('n_heads')}")
        ...
        # Our FSDP checkpoints have clean keys already, but strip module./
        # _orig_mod. defensively in case of DDP or torch.compile artifacts.
        clean_state_dict = {}
        for key, value in raw_state_dict.items():
            new_key = key.replace("_orig_mod.", "").replace("module.", "")
            clean_state_dict[new_key] = value

        # 5. Load weights directly into model.model (the raw Transformer), matching
        # the key format used by the reference WebOrganizer conversion script.
        # This avoids the brittle model. prefix manipulation and catches shape mismatches.
        ...
        if load_result.missing_keys:
            print(f"⚠️  Missing keys (in model but not checkpoint): {load_result.missing_keys[:10]}")
        unexpected = [k for k in load_result.unexpected_keys if "inv_freq" not in k]
        if unexpected:
            print(f"⚠️  Unexpected keys (in checkpoint but not model): {unexpected[:10]}")
...
print(f"\n🎉 ALL DONE! Your converted models are ready in:\n{OUTPUT_ROOT}")